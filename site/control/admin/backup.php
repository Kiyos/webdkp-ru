...
class pageBackup extends pageAdminMain {
	...
	/*=================================================
	Uploads a 'mini' lua file restore. This allows log files
	that are ussually only used to upload awards to be used
	...
	function RestoreDkpTableStage(&$entry){
		...
		//$this->log .= "Loaded player $name with id $playerid <br />";
	...
	function RestoreDkpHistoryStage(&$entry){
		...
		//find out the table that this entry should belong to
		//If the table isn't in our map - go ahead and create it
		$tableid = $this->tableMap[$tableid];
		if($tableid==0 || $tableid=="") {
			//if the table was not in the map... create another one
			$table = new dkpPointsTable();
			$table->guild = $this->guild->id;
			$table->name = "Unknown";
			$table->tableid = $this->updater->GetNextTableId();
