...
function WebDKP_AwardItem_Event()
	...
	if ( player == nil or player == "") then
		...
		WebDKP_UpdateTable();
		-- Check the sycnh stuff to see if we should forward this change to any other players, added by Zevious
		if WebDKP_Options["EnableSynch"] == 1 then			-- Make sure we have synching enabled
		
		-- Call the in game synching code and send the award to the addon channel.
...
function WebDKP_AwardZeroSum(points, reason, date)
	...
	-- Use existing code
	WebDKP_AddDKP(toAward, reason, "false", WebDKP_PlayersInGroup)
