...
class pageDkpMain extends page {
	...
	function LoadPageDetails(){
		...
		// For the personal addition the base url for the guild is always
		// the site root. This is because there is only one guild.
		$this->baseurl = $GLOBALS["SiteRoot"];
		...
		$this->setHeaderFooterVar("activetab", $this->tab);
		$this->setHeaderFooterVar("settings", $this->settings);
	}
	...
	function GetTabs($active="dkp"){
		...
		//If you wish old style tabs, uncomment the following line
	...
	function GetServer(){
		...
		$server->loadFromDatabaseByName($this->guild->server);
		return $server;
	}

	function GetGuild(){
		$guild = new dkpGuild();
		$guild->loadFromDatabase($this->settings->GetGuild());
		return $guild;
	}
	...
	function GetDKPFilterUI($prefix = ""){
		unset($template);
		$template = new template("site/control/bin/templates/dkpfilter.tmpl.php");
