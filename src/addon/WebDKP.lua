...
function WebDKP_OnEvent(self,event, ...)
   ...
   if(WebDKP_Options["Enabled"] == 1) then
        ...
        elseif(event=="COMBAT_LOG_EVENT_UNFILTERED") then
           WebDKP_BossAward_PerformAward(arg2,arg7);
...
-- =============================================
-- Called by the addon to initialize the dropdown. 
-- =============================================
...
-- ===========================================================================================
-- Notifies the player with the master table to start synching.
-- Added by Zevious (Bronzebeard)
-- ===========================================================================================
...
if WebDKP_Log ~= nil then
	...
	for i=1, 23, 1 do
		...
		if ( index <= numEntries) then
			...
			line:Show();
			awardedText:SetText(entries[index][4]);
...
function WebDKP_UpdateAwardedTable(reason,date)
	...
	for i=1, 23, 1 do
		...
		if ( index <= numEntries) then
			...
			line:Show();
			awardedText:SetText(nameentries[index][1]);
...
function WebDKP_SetItemSlotMulti(value, itemdesc)
	...
	elseif itemdesc == "Offhand Slot" then
