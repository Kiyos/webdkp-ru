...
if WebDKP_CharRaidInfo ~= nil then
	...
	for i=1, 25, 1 do
		...
		if ( index <= numEntries) then
			...
			totalraidsText:SetText(entries[index][2]);
			attendedraidsText:SetText(entries[index][3]);
			percentText:SetText(entries[index][4]);
		else
			-- if the line isn't in use, hide it so we dont' have mouse overs
			line:Hide();
		end
...
function WebDKP_UpdateCharAttendedTable(name)
	...
	for i=1, 25, 1 do
		...
		if ( index <= numEntries) then
			line:Show();
			--stringdata = raidsattended[index];
			--raidlocation1, raiddate1 = strsplit("&", stringdata)			
			locationText:SetText(entries[index][1]);
			dateText:SetText(entries[index][2]);
...
function WebDKP_UpdateAttendedTable(raid,starttime)
	...
	for i=1, 25, 1 do
		...
		if ( index <= numEntries) then
			line:Show();
			attendedText:SetText(nameentries[index]);
