...
function WebDKP_Synch_Processing(arg1,arg2)
	...
	-- This will process incoming dkp changes.
	if (string.find(cmd,"!SendingAuto")==1) and (string.find(cmd,synch_pass)==14) and enabled == 1 then
		local pattern = "!SendingAuto (%a+) (%a+),(%a+),(.+)";
		_, _,test, nameofchar, value, dkp = string.find(cmd, pattern);
		dkp = tonumber(dkp);
...
if enabled == 1 then
	...
	if foundBackup == 1 then
		...
		if (string.find(command,"!WebDKPAuto Item")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto Mult Start")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto MultNames")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto Mult End")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto Undo")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto LogAdd")==1) then
		...
		elseif (string.find(command,"!WebDKPAuto LogDel")==1) then
...
function WebDKP_Synch_Auto(points, foritem, players, reason, date)
	...
	if foritem == "UNDO" and reason ~= nil and date ~= nil then				-- This processes awards involved with an item
		SendAddonMessage("!WebDKPAuto Undo", reason..","..date, "GUILD");
	...
	elseif foritem == "LOGADD" and reason ~=nil and date ~= nil then
		...
		SendAddonMessage("!WebDKPAuto LogAdd", reason..","..date..","..players..","..points, "GUILD");
	elseif foritem == "LOGDEL" and reason ~=nil and date ~=nil then
		...
		SendAddonMessage("!WebDKPAuto LogDel", reason..","..date..","..players..","..points, "GUILD");		
	else
	...
	if foritem == "true" and players ~= nil then				-- This processes awards involved with an item
		SendAddonMessage("!WebDKPAuto Item", points..","..playerName..","..reason..","..date, "GUILD")
	end

	if players ~= nil and foritem == "false" then				-- This processes awards
		SendAddonMessage("!WebDKPAuto Mult Start", points..","..reason..","..numAwarded..","..date, "GUILD")	-- Announce the start of a multiple award so it monitors for names
		...
		for i=2, numAwarded, 1 do				-- This loop goes through the player list up to 10 to send the list of names
			...
			if countHolder == 10 then
				SendAddonMessage("!WebDKPAuto MultNames", CharList1, "GUILD")		-- Announce the first 10 awardees
				i = i + 1;
		...
		SendAddonMessage("!WebDKPAuto Mult End", "", "GUILD")					-- Announce the end of a multiple award	
	end
